...
import scoresys.javabean.Complaint;
 ...
 */
@WebServlet("/selectCommInfo")
//上面的是jsp界面form表单所抵达的地址
//查看投诉编号和投诉内容
public class selectCommInfo_servlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      //4- SQL语句
      String SQL = "SELECT * FROM Complaint "; // 进行数据库操作
      //5-执行SQL,返回数据
      ...
      while (rs.next()) {
        ...
        System.out.println(rs.getString(1) + "," + rs.getString(2));//查看投诉编号和投诉内容
        list.add(temp);
        //一行多少个值，下标就可以取到多少.
      }
      ...
      request.getRequestDispatcher("/Administrator_Home/pages/home/shenqingtudi.jsp").forward(request, response);//返回管理员首页
