...
import scoresys.javabean.Server;
...
@WebServlet("/customer")
public class Server_Home_Servlet extends HttpServlet {

    //        // TODO Auto-generated constructor stub
    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<Server> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
        ...
        try {
            ...
            //4- SQL语句
            String SQL = "SELECT Server_Id,PasswordFROM Server"; // 进行数据库操作
            //5-执行SQL,返回数据
            rs = stmt.executeQuery(SQL);
            //6-遍历
            while (rs.next()) {
                Server temp = new Land();
