...
@WebServlet("/farmer")
public class Farmer_Home_Servlet extends HttpServlet {

    //        // TODO Auto-generated constructor stub
    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<Farmer> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
        List<Farmer> list1 = new ArrayList<>();
        List<Farmer> list2 = new ArrayList<>();
        ...
        try {
            ...
            //4- SQL语句
            String SQL = "SELECT Farmer_Id,Password,Farmer_Phone FROM Farmer"; // 进行数据库操作
            //5-执行SQL,返回数据
            rs = stmt.executeQuery(SQL);
            //6-遍历
            while (rs.next()) {
                Customer temp = new Land();
                System.out.println(rs.getString(1) + "," + rs.getString(2));
                list.add(temp);
                //一行多少个值，下标就可以取到多少.
            }
            while (rs.next()) {
                Farmer temp = new Land();
                System.out.println(rs.getString(1) + "," + rs.getString(2));
                list1.add(temp);
                //一行多少个值，下标就可以取到多少.
            }
            ...
            request.getRequestDispatcher("/Personal_Information/pages/gerenxinxi/gerenxinxi.jsp").forward(request, response);
