 ...
 */
@WebServlet("/selectLeasedLandPopleInfo")
//上面的是jsp界面form表单所抵达的地址
//查看已出租土地ID和其租用者的信息
public class selectLeasedLandPopleInfo_servlet extends HttpServlet {
  //        // TODO Auto-generated constructor stub
  @Override
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      List<Leased_land> list = new ArrayList<Leased_land>(); //用于存储数据，类写在javabean里面，记得改一下
      List<Leased_land> list1 = new ArrayList<Leased_land>(); //
      List<Leased_land> list2 = new ArrayList<Leased_land>(); //
      ...
      try {
        ...
        //4- SQL语句
        String SQL = "SELECT * FROM Leased_land";
        //5-执行SQL,返回数据
        rs = stmt.executeQuery(SQL);
        //6-遍历
        while (rs.next()) {
          Leased_land temp = new Leased_land();
          System.out.println(rs.getString(6) +"," + rs.getString(4));//查看已出租土地ID和其租用者的信息
          list.add(temp);
          //一行多少个值，下标就可以取到多少.
        }
        while (rs.next()) {
      ...
      request.setAttribute("lard", list2); // 现在跳转到下一个界面还是存在的
        request.getRequestDispatcher("/Leased_land/pages/home/yichuzutudiliebiao.jsp").forward(request, response);
