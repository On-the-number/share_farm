...
public class Customer_Dao {
  public List getall() {
    List<Customer> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
    ...
    try {
      ...
      while (rs.next()) {
        ...
        temp.setAGE(Integer.parseInt(rs.getString(2)));
        temp.setCustomer_mail(rs.getString(3));
        temp.setCustomer_Name(rs.getString(4));
        temp.setCustomer_Phone(rs.getString(5));
        temp.setCustomer_Sex(rs.getString(6));
        temp.setNum(Integer.parseInt(rs.getString(7)));
        temp.setPassword(rs.getString(8));
        list.add(temp);
        //一行多少个值，下标就可以取到多少.
      }
