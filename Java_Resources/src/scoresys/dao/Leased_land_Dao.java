...
import java.util.List;
  ...
  public class Leased_land_Dao {
    public List getall() {
      List<Leased_land> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
      ...
      try {
        ...
        while (rs.next()) {
          ...
          temp.setCustomer_Id(rs.getString(3));
          temp.setEnd_data(rs.getString(4));
          temp.setFarmer_Id(rs.getString(5));
          temp.setFarmer_Id(rs.getString(6));
          temp.setFarmer_Id(rs.getString(7));
          list.add(temp);
          //一行多少个值，下标就可以取到多少.
        }
