...
public class Message_Dao {
  public List getall() {
    List<Message> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
    ...
    try {
      ...
      while (rs.next()) {
        ...
        temp.setMessage_Id(rs.getString(1));
        temp.setMessage_content(rs.getString(2));
        temp.setMessage_name(rs.getString(3));
        temp.setLand_id(rs.getString(4));
        list.add(temp);
        //一行多少个值，下标就可以取到多少.
      }
