...
public class CompLaint_Dao {
  public List getall() {
    List<Complaint> list = new ArrayList<>(); //用于存储数据，类写在javabean里面，记得改一下
    ...
    try {
      ...
      while (rs.next()) {
        ...
        temp.setComplaint_content(rs.getString(1));
        temp.setComplaint_Id(rs.getString(2));
        temp.setComplaint_name(rs.getString(3));
        temp.setComplaint_Phone(rs.getString(4));
        list.add(temp);
        //一行多少个值，下标就可以取到多少.
      }
